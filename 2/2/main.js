...
const rawstring = `493  458 321 120 49  432 433 92  54  452 41  461 388 409 263 58
...
const checksum = data.map(list =>
{
    const isDivisable = (divised, divisor) => divised % divisor === 0 && divised !== divisor;
    list = list.sort();
    reverseList = [...list].reverse();

    for (smallElement of list)
        for (bigElement of reverseList)
            if (isDivisable(bigElement, smallElement))
                return  bigElement / smallElement; 
})
.reduce((a,b) => a + b);
